...
| D19 | 25-09-2026 | **Economía de las tareas** (fase 14), elegida tras un análisis de Habitica, Todoist, economías de fichas y economía de juegos: pequeña 5, mediana 10, grande 20, con **tope de 30 pts por día**; un solo movimiento `task_completion` por día; no tocan racha ni día perfecto; vencidas sin castigo; se borran salvo las cumplidas en días cerrados | Los puntos de los hábitos no cambian ahora: se revisan en la calibración del ~13-10, incluida la idea de que un hábito valga menos después de sus primeros ~66 días. Detalle en [14-task-tracker.md](14-task-tracker.md) |
| D20 | 25-09-2026 | **Hábitos no diarios y con cantidad** (fase 16): días fijos (si un día no tiene principales, la meta son los que tocan); veces por semana con N elegido (1 a 6), que **no toca la racha** y suma hasta N por semana; con todos los semanales cumplidos, la **llama se vuelve morada** hasta el domingo en vez de llevar otra racha; cantidad con meta del día sin puntos parciales; frecuencia y meta **fijas al crear** | Sin estado nuevo: la semana potenciada se deriva de las marcas. Detalle en [16-flexible-habits.md](16-flexible-habits.md) |
...
| 12 | **hecha en web** | unida a `main` | Aprobada por el usuario (25-09-2026), **sin publicar** en producción todavía (decisión del usuario). Racha en riesgo con cuenta regresiva en Hoy e insignias de hitos (7/30/100/365) con su celebración y tarjeta en Año. Sin cambios de datos ni reglas. En el celular: con el próximo build |
| 13 | **hecha en web** | unida a `main` | Aprobada por el usuario (25-09-2026), **sin publicar** en producción. Sonidos sintetizados con `expo-audio` e interruptor en Ajustes, llama viva (SVG + Reanimated) en la celebración y el brasero, deslizar una fila para marcarla. Sin cambios de datos ni reglas; suma un módulo nativo (`expo-audio`) para el próximo build |
| 14 | **hecha en web** | unida a `main` | Aprobada por el usuario (25-09-2026), **sin publicar** en producción. Tareas con la economía D19 (5/10/20, tope de 30 por día): sección en Hoy con formulario y menú, puntos al cerrar el día, un donut por día en Mes → Semana y tareas en la demo. **Toca reglas:** al publicar, reglas antes que la web |
| 16 | **en curso** | `feat/16-flexible-habits` | Hábitos de días fijos, veces por semana y con cantidad (D20). Plan y decisiones en [16-flexible-habits.md](16-flexible-habits.md). **Toca reglas** |

## Bitácora

Lo más reciente arriba. Una línea por sesión: fecha, máquina (oficina/casa), qué se hizo y qué queda a medias.

- **25-09-2026 · oficina** — Probada la ruta de `npm run demo` con los emuladores ya encendidos (`npm run emulators` antes): los usa, siembra y al cerrar los deja encendidos. Arreglo: con el puerto 8082 ocupado (otra demo abierta), Expo se cerraba sin servidor **después** de volver a sembrar encima de esa demo; ahora la demo lo comprueba primero ocupando el puerto (un `fetch` no sirve: Metro tarda más de 1,5 s en responder) y se niega sin tocar nada. El usuario aprobó la fase 12: unida a `main` sin publicar en producción. Fase 13 en `feat/13-sounds-flame-swipe` ([13-sounds-flame-swipe.md](13-sounds-flame-swipe.md)): sonidos sintetizados por script (`expo-audio`, sin micrófono ni segundo plano en el plugin) con interruptor por dispositivo, llama viva con SVG + Reanimated en lugar de Lottie (sin dependencias nuevas), deslizar para marcar. Revisado con Edge headless contra una demo propia (la del usuario se cerró con su permiso). Tests: shared 227, reglas 157, cliente 129. El usuario la aprobó: unida a `main` sin publicar. Queda verla en el celular con el próximo build. Fase 14 en `feat/14-task-tracker` ([14-task-tracker.md](14-task-tracker.md)): el usuario pidió analizar la economía antes de fijar el tope; con un análisis (Habitica, Todoist, economías de fichas, economía de juegos) eligió tareas 5/10/20 con tope de 30 por día y dejar los hábitos para la calibración (D19). Hechos con TDD el núcleo, las reglas (verificadas con mutaciones) y las operaciones. Los tests de reglas se corrieron en un emulador aparte (puerto 8180) porque la demo del usuario ocupaba el 8080. Después, la app: sección Tareas en Hoy (vencidas, de hoy, cumplidas, próximas; tocar o deslizar), formulario con tamaño y días rápidos, menú (editar, mover a mañana, borrar), puntos de tareas en +N hoy, y tareas en la demo; revisado con Edge headless en claro y oscuro (la demo del usuario se cerró con su permiso). Luego, Mes → Semana: un donut por día (SVG propio) y, al tocarlo, las tareas del día; cada tarea cuenta en un solo día, el que se cumplió o, si sigue pendiente, el que vence (`taskDays`, TDD). Revisado en la demo del usuario sin tocar sus datos (solo lectura). Tests: shared 257 (100 %), reglas y operaciones 182, cliente 147. El usuario la aprobó: unida a `main` sin publicar (al publicar, reglas antes que la web). Siguiente, elegida por el usuario: fase 16, hábitos no diarios y con cantidad.
- **24-09-2026 · oficina** — En `feat/12-streak-risk-milestones`: **`npm run demo`** (emuladores + siembra + web en `:8082`, Ctrl+C lo apaga y borra todo; sin producción, elegido por el usuario) con opciones `--streak=N` y `--risk` ([08-statistics.md](08-statistics.md#cómo-probarlo-con-datos)). Probado: racha 6 → franja de riesgo → marcar principales → celebración con insignia de 7; siembras 0/1/29/99/364 dan la racha exacta; cierre de Expo apaga Java. Arreglo encontrado con la demo: la vista Año y la tarjeta de insignias usan la racha en vivo de hoy (`buildTodaySummary.longestStreak`), antes decían "te falta 1 día" justo después de celebrar. Sin aviso del emulador de Auth en web. Queda a medias: probar a mano la ruta "emuladores ya encendidos" de `npm run demo`. Sigue pendiente el visto bueno de la fase 12.

- **24-09-2026 · oficina** — El usuario pidió no publicar todavía la fase 11 en producción y empezar la 12. Fase 12 en `feat/12-streak-risk-milestones`: `streakRiskAt`/`formatTimeLeft` y `milestoneReached`/`milestoneProgress` en `shared` con TDD (227, 100 %), franja de racha en riesgo en Hoy, insignias SVG por hito, celebración de hito y tarjeta de insignias en Año. Revisado con Edge headless contra emuladores; reglas 157, cliente 110. Queda: revisión del usuario (canal de vista previa) y unir a `main`.
...
- **23-09-2026 · oficina** — Fase 08 y revisión de paleta aprobadas por el usuario. Typecheck, lint y tests en verde (shared 164, reglas 151, cliente 81). Publicadas las reglas y luego la web; `feat/08-statistics` y `feat/01-palette-categories` unidas a `main` (fast-forward). Siguiente: fase 09 (primera APK, lo pendiente de Android de 05, 06 y 08, y recordatorios locales); depende del cupo mensual de EAS.
  ...
  Build de la APK lanzado en EAS (nube, sin EAS Update por decisión del usuario; el plan gratis da 15 builds de Android al mes). Mientras tanto, fase 10 en `feat/10-export`: exportación JSON/CSV (`shared/export.ts`, `data/export-files.ts`, sección "Tus datos"), probada en web (shared 197, reglas 157, cliente 85). `expo-sharing` es nativo: la exportación en Android llega con el próximo build. Siguiente: modo oscuro.
