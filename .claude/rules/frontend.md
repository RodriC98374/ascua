...
| `features/<dominio>/` | Todo lo de un dominio: componentes, hooks y lógica de UI con sus tests (`auth`, `habits`, `today`, `close-day`, `rewards`, `statistics`, `sync`, `reminders`, `export`, `appearance`, `sounds`…) | Una funcionalidad nueva = una carpeta nueva aquí |
