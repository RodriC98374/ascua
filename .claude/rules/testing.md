...
- Las operaciones de la app (`initializeAccount`, `closePendingDays`, compras, canjes) se prueban contra el emulador, nunca contra el proyecto real, incluyendo concurrencia (dos ejecuciones en paralelo). Viven en `apps/client/src/operations/` con imports relativos y sus tests en `packages/firestore-rules/src/operations/`.
